				 ...
				 struct snd_soc_dai *dai)
{
	...
	unsigned int rate_reg = mt6991_rate_transform(afe->dev, rate, dai->id);
	unsigned int x_rate_reg = pcm_1x_rate_transform(afe->dev, rate);
	unsigned int pcm_con0 = 0;
	unsigned int pcm_con1 = 0;

	dev_info(afe->dev, "%s(), id %d, stream %d, rate %d, rate_reg %d\n",
		 __func__,
		 dai->id,
		 substream->stream,
		 rate,
...
int mt6991_dai_pcm_register(struct mtk_base_afe *afe)
{
