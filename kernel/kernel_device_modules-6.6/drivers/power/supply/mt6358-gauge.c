...
static void pre_gauge_update(struct mtk_gauge *gauge)
{
	...
	do {
		m++;
		if (m > 1000) {
			bm_err(gauge->gm, "[%s] gauge_update_polling timeout 1:%x!\r\n",
				__func__, reg_val);
			break;
		}
...
static void post_gauge_update(struct mtk_gauge *gauge)
{
	...
	do {
		m++;
		if (m > 1000) {
			bm_err(gauge->gm, "[%s] gauge_update_polling timeout 2!\r\n",
				__func__);
			break;
		}
		regmap_read(gauge->regmap, RG_FGADC_CON1, &reg_val);
...
static int reg_to_current(struct mtk_gauge *gauge, unsigned int regval)
{
	...
	bm_debug(gauge->gm, "[%s] 0x%x 0x%x 0x%x 0x%x 0x%x %d\n",
		__func__,
		regval, uvalue16, dvalue, (int)temp_value,
		retval, is_charging);

	if (is_charging == false) {
		bm_err(gauge->gm, "[%s], is_charging=%d\n", __func__, is_charging);
		return -retval;
	}
...
static u8 get_rtc_spare0_fg_value(struct mtk_gauge *gauge)
{
	...
	if (IS_ERR(cell)) {
		bm_err(gauge->gm, "[%s]get rtc cell fail\n", __func__);
		return 0;
	}
	...
	nvmem_cell_put(cell);
	if (IS_ERR(buf)) {
		bm_err(gauge->gm, "[%s]read rtc cell fail\n", __func__);
		return 0;
	}
...
static void set_rtc_spare0_fg_value(struct mtk_gauge *gauge, u8 val)
{
	...
	nvmem_cell_put(cell);
	if (ret != length)
		bm_err(gauge->gm, "[%s] write rtc cell fail\n", __func__);
}
	...
	enum gauge_property ginfo, unsigned int value)
{
	...
	bm_debug(gauge->gm, "[%s]info:%d v:%d\n", __func__, ginfo, value);
	...
	int _prd, int _zcv_mv, int _thr_mv)
{
	...
	bm_err(gauge->gm, "[fg_bat_nafg][fgauge_set_nafg_interrupt_internal] time[%d] zcv[%d:%d] thr[%d:%d] 26_16[0x%x] 15_00[0x%x]\n",
	...
	struct mtk_gauge_sysfs_field_info *attr, int *zcv_current)
{
	...
	if (gauge->gm->fg_cust_data.r_fg_value != DEFAULT_R_FG) {
		bm_debug(
		gauge->gm, "[fgauge_read_current] Auto adjust value due to the Rfg is %d\n Ori curr=%d",
	...
	int fg_zcv_det_time, int fg_zcv_car_th)
{
	...
	bm_debug(gauge_dev->gm, "[FG_ZCV_INT][%s] det_time %d mv %d reg %lld 30_16 0x%x 15_00 0x%x\n",
...
int nafg_check_corner(struct mtk_gauge *gauge)
{
	...
	bm_debug(gauge->gm, "%s:corner:%d nag_vbat:%d nag_zcv:%d get_c_dltv_mv:%d setto_cdltv_thr_mv:%d, diff:%d, RG[0x%x,0x%x]\n",
...
void iavg_workaround(struct mtk_gauge *gauge, enum gauge_event evt)
{
	...
	bm_err(gauge->gm, "[%s]type:%d 0x%x 0x%x!\n", __func__, evt, dwa, fg_int_mode);
}
...
int fgauge_get_time(struct mtk_gauge *gauge_dev, unsigned int *ptime)
{
	...
	bm_debug(
		 gauge_dev->gm, "[%s] low:0x%x high:0x%x rtime:0x%llx 0x%x!\r\n",
	...
	struct mtk_gauge_sysfs_field_info *attr, int *data)
{
	...
	if (iavg_vld == 1) {
		...
		bm_debug(
			gauge_dev->gm, "[fg_get_current_iavg] fg_iavg_ma %lld fg_iavg_reg %lld fg_iavg_reg_tmp %lld\n",
	...
	struct mtk_gauge_sysfs_field_info *attr, int *val)
{
	...
	/*Auto adjust value*/
	if (r_fg_value != DEFAULT_R_FG) {
		bm_debug(gauge->gm, "[%s] Auto adjust value due to the Rfg is %d\n Ori CAR=%d",
			 __func__,
	...
	struct mtk_gauge_sysfs_field_info *attr, int en)
{
	...
	if ((is_iavg_valid == 1) && (gauge_status->iavg_intr_flag == 0)) {
		...
		disable_gauge_irq(gauge_dev, FG_IAVG_L_IRQ);
		bm_debug(
			gauge_dev->gm, "[read_fg_hw_info] doublecheck first fg_set_iavg_intr %d %d\n",
			is_iavg_valid, gauge_status->iavg_intr_flag);
	}
...
static int calculate_car_tune(struct mtk_gauge *gauge)
{
	...
	if (gauge->hw_status.meta_current != 0) {
		...
		bm_err(gauge->gm, "[555]Temp_Value2 %lld current_from_ADC %lld UNIT_FGCURRENT %d\n",
			Temp_Value2, current_from_ADC, UNIT_FGCURRENT);
		...
		bm_err(gauge->gm, "[666]Temp_Value2 %lld current_from_ADC %lld UNIT_FGCURRENT %d\n",
			Temp_Value2, current_from_ADC, UNIT_FGCURRENT);
...
static int get_ptim_current(struct mtk_gauge *gauge)
{
	...
	/* Check (-1) */
	dvalue = dvalue * -1;
	bm_err(gauge->gm, "[%s]ptim current:%d\n", __func__, dvalue);
	...
	struct mtk_gauge_sysfs_field_info *attr, int *nag_cnt)
{
	...
	*nag_cnt = (NAG_C_DLTV_CNT & AUXADC_NAG_CNT_15_0_MASK) +
	...
	bm_debug(gauge->gm, "[fg_bat_nafg][%s] %d [25_16 %d 15_0 %d]\n",
	...
	struct mtk_gauge_sysfs_field_info *attr, int *nag_dltv)
{
	...
	*nag_dltv = nag_dltv_mv_value;
	...
	bm_debug(gauge->gm, "[fg_bat_nafg][%s] mV:Reg [%d:%d] [%d:%d]\n",
		...
		reg_to_mv_value(gauge, reg_value), reg_value);
	...
	struct mtk_gauge_sysfs_field_info *attr, int *nafg_c_dltv)
{
	...
	if (gauge->nafg_corner == 1) {
		...
		*nafg_c_dltv = nag_c_dltv_mv_value;
		...
		bm_debug(gauge->gm, "[fg_bat_nafg][%s] mV:Reg[%d:%d] [b10:%d][26_16(0x%04x) 15_00(0x%04x)] corner:%d\n",
		...
		nag_c_dltv_mv_value =
			reg_to_mv_value(gauge, nag_c_dltv_reg_value);
		*nafg_c_dltv = nag_c_dltv_mv_value;
	...
	struct mtk_gauge_sysfs_field_info *attr, int *val)
{
	...
	*val = _hw_ocv;
	...
	bm_err(gm, "[%s] _hw_ocv_58_pon_rdy %d _hw_ocv_58_pon %d _hw_ocv_58_plugin %d _hw_ocv_chgin %d _sw_ocv %d now_temp %d now_thr %d\n",
	...
	struct mtk_gauge_sysfs_field_info *attr, int val)
{
	...
	bm_err(gauge->gm, "set RG_FG_OFFSET_SWAP orig %x,new %x\n", temps, rev_val);
	...
	bm_err(gauge->gm, "set PMIC_RG_LDO_VBIF28_EN from %d,to 1\n", vbif28en);
	...
	bm_err(gauge->gm, "bat_plug:%d chr:%d info:0x%x, slp_cur_th:%x\n",
	...
	struct mtk_gauge_sysfs_field_info *attr, int val)
{
	...
	bm_debug(gauge->gm, "[%s] ui_soc=%d, spare3_reg=0x%x, valid:%d, new_spare3_reg:0x%x\n",
	...
	struct mtk_gauge_sysfs_field_info *attr, int threshold)
{
	...
	bm_debug(gauge->gm, "[fg_set_vbat2_h_th] thr:%d [0x%x %d 0x%x 0x%x %d 0x%x]\n",
	...
	struct mtk_gauge_sysfs_field_info *attr, int threshold)
{
	...
	bm_debug(gauge->gm, "[fg_set_vbat2_l_th] thr:%d [0x%x %d 0x%x 0x%x %d 0x%x]\n",
...
void dump_nag(struct mtk_gauge *gauge)
{
	...
	bm_err(gauge->gm, "nag %d %d %d %d %d %d %d %d %d %d\n",
...
struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	bm_notice(gm, "%s 32bit IOCTL, cmd=0x%08x\n",
		__func__, cmd);
	if (!filp->f_op || !filp->f_op->unlocked_ioctl) {
		bm_err(gm, "%s file has no f_op or no f_op->unlocked_ioctl.\n",
			__func__);
		return -ENOTTY;
	}

	if (sizeof(arg) != sizeof(adc_out_datas)) {
		bm_err(gm, "%s sizeof(arg)=%zu sizeof(adc_out_data)=%zu\n",
	...
	case Set_CARTUNE_TO_KERNEL: {
		...
		break;
	default:
		bm_err(gm, "%s unknown IOCTL: 0x%08x, %d\n",
	...
	struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case Get_META_BAT_VOL:
		adc_out_data[0] =
			gauge_get_int_property(gm, GAUGE_PROP_BATTERY_VOLTAGE);
		if (copy_to_user(user_data_addr, adc_out_data,
			sizeof(adc_out_data))) {
			bm_err(gm, "%s copy META_BAT_VOL to user fail\n", __func__);
			mutex_unlock(&gm->gauge->fg_mutex);
			return -EFAULT;
		}

		bm_notice(gm, "**** unlocked_ioctl :Get_META_BAT_VOL Done!\n");
		break;
	case Get_META_BAT_SOC:
		...
		if (copy_to_user(user_data_addr, adc_out_data,
			sizeof(adc_out_data))) {
			bm_err(gm, "%s copy META_BAT_SOC to user fail\n", __func__);
			mutex_unlock(&gm->gauge->fg_mutex);
			return -EFAULT;
		}

		bm_notice(gm, "**** unlocked_ioctl :Get_META_BAT_SOC Done!\n");
		break;
	...
	case Get_META_BAT_CAR_TUNE_VALUE:
		...
		if (copy_to_user(user_data_addr, adc_out_data,
			sizeof(adc_out_data))) {
			bm_err(gm, "%s copy META_BAT_CAR_TUNE_VALUE to user fail\n", __func__);
			mutex_unlock(&gm->gauge->fg_mutex);
			return -EFAULT;
		}

		bm_notice(gm, "**** unlocked_ioctl :Get_META_BAT_CAR_TUNE_VALUE Done!\n");
		break;
	case Set_META_BAT_CAR_TUNE_VALUE:
		...
		if (copy_to_user(user_data_addr, adc_out_data,
			sizeof(adc_out_data))) {
			bm_err(gm, "%s copy Set_META_BAT_CAR_TUNE_VALUE to user fail\n", __func__);
			mutex_unlock(&gm->gauge->fg_mutex);
			return -EFAULT;
		}

		bm_err(gm, "**** unlocked_ioctl Set_BAT_CAR_TUNE_VALUE[%d], tmp_car_tune=%d result=%d, ret=%d\n",
	...
	case Set_BAT_DISABLE_NAFG:
		...
		bm_debug(gm, "unlocked_ioctl Set_BAT_DISABLE_NAFG,isdisNAFG=%d [%d]\n",
			isdisNAFG, adc_in_data[1]);
		break;
