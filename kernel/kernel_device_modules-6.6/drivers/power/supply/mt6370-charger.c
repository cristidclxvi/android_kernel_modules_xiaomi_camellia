...
static const struct linear_range mt6370_chg_ranges[MT6370_RANGE_F_MAX] = {
	...
	LINEAR_RANGE_IDX(MT6370_RANGE_F_AICL_VTH,4100000, 0x0, 0x07, 100000),
	LINEAR_RANGE_IDX(MT6370_RANGE_F_PUMPX,5500000, 0x0, 0x07, 500000),
	LINEAR_RANGE_IDX(MT6370_RANGE_F_WT_FC,4, 0x0, 0x07, 2),
	LINEAR_RANGE_IDX(MT6370_RANGE_F_IR_CMP,0, 0x0, 0x07, 25000),
	LINEAR_RANGE_IDX(MT6370_RANGE_F_VCLAMP,0, 0x0, 0x07, 32000),
};
...
static int mt6370_chg_enable_bc12(struct mt6370_priv *priv, bool en)
{
	...
	if (en) {
		...
		if (i == max_wait_cnt)
			dev_notice(priv->dev, "%s : CDP timeout\n", __func__);
		else
			dev_info(priv->dev, "%s : CDP free\n", __func__);
	}
				   ...
				   const char *buf, size_t count)
{
	...
	mt6370_enable_hidden_mode(priv, true);
	ret = regmap_clear_bits(priv->regmap, MT6370_REG_CHGHIDDENCTRL15, 0x01);
	if (ret < 0) {
	...
	mdelay(50);
	/* enter shipping mode */
	ret = regmap_set_bits(priv->regmap, MT6370_REG_CHG_CTRL2, 0x80);
	if (ret < 0) {
