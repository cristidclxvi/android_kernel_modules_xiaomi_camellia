				    ...
				    int fps_n, int fps_d, s64 *result)
{
	...
	*result = buffered_pixel_rate;
	...
	seninf_logd(ctx,
		"w %llu h %llu hb %llu vb %llu fps %d/%d pclk %llu->%llu orig %llu k %llu hbe %d\n",
	...
	if (core->refcnt > 0) {
		...
		if (core->refcnt == 1) {
			if (core->pwr_refcnt_for_aov &&
				...
				!(ctx->current_sensor_id < 0) &&
				(ctx->current_sensor_id != core->aov_sensor_id))
				seninf_logd(ctx, "aov sensor streaming on scp now, won't disable mux/cammux\n");
			else {
				seninf_logd(ctx, "common sensor streaming, disable mux/cammux for initialization\n");
